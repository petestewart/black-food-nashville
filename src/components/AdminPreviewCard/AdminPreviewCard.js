        ...
        <div className="">
          ...
          <div className="rest-subheading">
            ...
            <div className="approvalButtons d-flex justify-content-center w-100 p-3">
              <button className={`btn btn-${rest.DELETE ? 'danger' : 'outline-success'} mr-2`} onClick={() => props.approveSubmission(rest)}>Approve</button>
              <button className={`btn btn-outline-${rest.DELETE ? 'secondary' : 'danger'} ml-2`} onClick={() => props.denySubmission(rest)}>Deny</button>
            </div>
          </div>
        </div>
      </div>
